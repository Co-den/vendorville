...
type CartItem = { productId: number; name: string; price: number; quantity: number; stock: number };
...
export default function StorefrontPage() {
  ...
  const { business, products, isLoading, isSubmitting, error, fetchStorefront, createOrder, verifyPayment } = useStorefrontStore();
  ...
  if (error || !business) {
    return (
      <div style={{ textAlign: "center", padding: "80px 24px" }}>
        <h1 style={{ fontFamily: "var(--font-fraunces), serif", fontSize: "1.5rem" }}>Store not found</h1>
        <p style={{ color: "var(--gray)", marginTop: 8 }}>{error || "This store may no longer be available."}</p>
      </div>
    );
  }
  ...
  const images = business.premisesImages.length > 0 ? business.premisesImages : [];
  ...
  return (
    ...
    <div className="storefront-page">
      <div className="sf-banner">
        ...
        <button className="sf-back-btn" onClick={() => router.push("/discover")}>
        ...
        <span className={`sf-status-badge ${business.isOpenToday ? "open" : "closed"}`}>
      ...
      <div className="sf-header-row">
        <div className="sf-header-left">
          <div className="sf-logo">
            {business.logoUrl ? <img src={business.logoUrl} alt={business.name} /> : business.name[0]}
          </div>
          <div>
            <h1>{business.name}</h1>
            <div className="sf-rating">☆☆☆☆☆ <b>0.0</b> (0 reviews)</div>
          </div>
        </div>
        <button className="storefront-cart-btn" onClick={() => setShowCheckout(true)} disabled={cart.length === 0}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="9" cy="21" r="1" /><circle cx="20" cy="21" r="1" />
            <path d="M1 1h4l2.68 13.39a2 2 0 002 1.61h9.72a2 2 0 002-1.61L23 6H6" />
          </svg>
          Cart ({cart.reduce((s, i) => s + i.quantity, 0)}) · ₦{cartTotal.toLocaleString()}
        </button>
      </div>
      ...
      <div className="sf-body">
        <div className="sf-main">
          <div
            className="sf-slider"
            style={{ transform: `translateX(-${activeIndex * (100 / tabs.length)}%)` }}
          >
            {/* ===== OVERVIEW TAB ===== */}
            <div className="sf-slide">
              <div className="panel">
                ...
                <h3 style={{ fontSize: "0.8rem", color: "var(--gray)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  Products
                </h3>
              </div>

              {products.length === 0 ? (
                <p style={{ textAlign: "center", color: "var(--gray)", padding: "32px 0" }}>No products available right now.</p>
              ) : (
                <div className="storefront-product-grid" style={{ marginTop: 16 }}>
                  {products.map((product) => {
                    const inCart = cart.find((i) => i.productId === product.id);
                    return (
                      <div className="storefront-product-card" key={product.id}>
                        <div className="storefront-product-image">
                          {product.imageUrl ? <img src={product.imageUrl} alt={product.name} /> : <span>{product.name[0]}</span>}
                        </div>
                        <div className="storefront-product-info">
                          <div className="storefront-product-name">{product.name}</div>
                          <div className="storefront-product-price">₦{product.price.toLocaleString()}</div>
                          {inCart ? (
                            <div className="pos-qty-control" style={{ justifyContent: "center" }}>
                              <button type="button" onClick={() => updateQty(product.id, -1)}>−</button>
                              <span>{inCart.quantity}</span>
                              <button type="button" onClick={() => updateQty(product.id, 1)} disabled={inCart.quantity >= product.stock}>+</button>
                            </div>
                          ) : (
                            <button className="storefront-add-btn" onClick={() => addToCart(product)}>Add to Cart</button>
                          )}
                        </div>
                      </div>
                    );
            ...
            <div className="sf-slide">
              <div className="panel">
                <h2>Location</h2>
                <p style={{ color: "var(--ink)", fontWeight: 600, marginTop: 10 }}>{business.address}</p>
                <p style={{ color: "var(--gray)", fontSize: "0.86rem", marginTop: 14 }}>
            ...
            <div className="sf-slide">
              <div className="panel" style={{ textAlign: "center", padding: "48px 24px" }}>
                <p style={{ color: "var(--gray)" }}>No reviews yet. Be the first to order and leave feedback.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="panel">
            ...
            {business.businessEmail && (
              <div className="sf-contact-row">
                ...
                <div>
                  ...
                  <div className="sf-contact-value">{business.businessEmail}</div>
                </div>
              </div>
            )}
            ...
            {(business.instagram || business.tiktok) && (
              <div className="sf-social-row">
                <div className="sf-contact-label" style={{ marginBottom: 8 }}>Find Us On</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {business.instagram && <a href={`https://${business.instagram}`} target="_blank" className="sf-social-pill instagram">Instagram</a>}
                  {business.tiktok && <a href={`https://${business.tiktok}`} target="_blank" className="sf-social-pill tiktok">TikTok</a>}
                </div>
              </div>
            )}
            ...
            {business.whatsappNumber && (
               ...
               <a href={`https://wa.me/${business.whatsappNumber.replace(/\D/g, "")}`}
                target="_blank"
                className="btn-create"
                style={{ width: "100%", textAlign: "center", display: "block", marginTop: 20, textDecoration: "none" }}
              >
                Call for Inquiries
              </a>
            )}
        ...
        <div className="modal-overlay" onClick={() => setShowCheckout(false)}>
          <div className="modal-card tall" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>Checkout</h3>
              <button type="button" className="modal-close-btn" onClick={() => setShowCheckout(false)}>
                  ...
                  {cart.map((item) => (
                    <div className="pos-cart-item" key={item.productId}>
                      <div className="pos-cart-item-info">
                        ...
                        <div className="pos-cart-item-price">{item.quantity} × ₦{item.price.toLocaleString()}</div>
                      </div>
                ...
                <div className="wallet-balance-strip" style={{ marginTop: 16 }}>
                  <div>
                    <div style={{ fontSize: "0.8rem", color: "var(--gray)" }}>
                      Subtotal: ₦{cartTotal.toLocaleString()} + Delivery: ₦{Number(deliveryFee).toLocaleString()}
                    </div>
                    <div className="wallet-balance-strip-value">₦{grandTotal.toLocaleString()}</div>
                  </div>
                </div>
                {checkoutError && <div className="error-message" style={{ marginTop: 14 }}>{checkoutError}</div>}
              </div>
        ...
        <div className="modal-overlay" onClick={() => setOrderSuccess(null)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            <div className="modal-success">
              ...
              <h3>Order Placed!</h3>
              <p className="modal-sub">{orderSuccess.orderNumber} — {business.name} will reach out to confirm delivery.</p>
              <div className="modal-actions">
                <button className="btn-primary-modal" style={{ flex: "none", width: "100%" }} onClick={() => setOrderSuccess(null)}>Done</button>
              </div>
            </div>
          </div>
        </div>
